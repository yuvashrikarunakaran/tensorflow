...
TEST_F(CompositeInliningTest, CompositeDelegateWorks) {
  graph_builder::GraphBuilder graph_builder;
  auto primary_graph = graph_builder.NewGraph();
  auto decomposition_graph = graph_builder.NewGraph();
  {
  ...
  graph_builder.Build(interpreter_);
...
TEST_F(CompositeInliningTest, RecursiveInliningWorks) {
  graph_builder::GraphBuilder graph_builder;
  auto primary_graph = graph_builder.NewGraph();
  auto decomposition_graph_with_composite = graph_builder.NewGraph();
  auto supported_decomposition_graph = graph_builder.NewGraph();
...
TEST_F(CompositeInliningTest, InliningSharedDecompositionWorks) {
  ...
  {
    auto in1 = primary_graph.NewInput(kTfLiteInt32);
    auto in2 = primary_graph.NewInput(kTfLiteInt32);
    auto composite1 =
